...
const ReachOutSection = () => {
    ...
    emailjs
      .sendForm(
        process.env.NEXT_PUBLIC_SERVICE_ID ??
          (() => {
            throw new Error(
              "SERVICE_ID is not defined in environment variables"
            );
          })(), // Replace with your EmailJS service ID
        process.env.NEXT_PUBLIC_TEMPLATE_ID ??
          (() => {
            throw new Error(
              "Template ID is not defined in environment variables"
            );
        ...
        process.env.NEXT_PUBLIC_PUBLIC_KEY ??
          (() => {
            throw new Error(
              "Public Key is not defined in environment variables"
            );
  ...
  return (
    <div className="flex flex-col lg:flex-row py-[50px] gap-[50px]">
      <div className="flex-1 w-full flex flex-col gap-[12px]">
        ...
        <h1 className="font-helvetica font-light text-[18px] md:text-[24px] leading-[1.1]">
          Ready to Shape the Future of Dentistry? Apply today and let's make a
          difference together!
        </h1>
      </div>
        ...
        >
          <div className="w-full flex flex-col gap-1 md:gap-2">
            ...
            <input
              ...
              required
              className="w-full px-2 py-2 md:py-3 md:px-6 flex flex-col gap-[10px] border-[0.5px] border-black rounded-[8px] font-helvetica font-light text-[20px]"
            />
          </div>
          <div className="w-full flex flex-col gap-1 md:gap-2">
            <h1 className="font-helvetica font-normal text-[18px] md:text-[24px]">
